...
# =====> USER SETTINGS <=====
# Network setting                                         
layer_type  = 'DW'    # Options: 'PW', 'DW', 'LINEAR', 'CONV2D'  
IN_CH       = 64
INPUT_H     = 27                    
INPUT_W     = 7                       
KER_H       = 3                                          
...
if WRITE_YML_FILE == True:
    ...
    if layer_type == 'DW' or layer_type == 'PW':
        ...
        if layer_type == 'DW':
            ...
            if IGNORE_FW == False:
                for idx in range(NUM_FOUND_SOLUTIONS):
                    dw_template(num_cores=1, step='FW')
            # WGT_G 1 Core
            if IGNORE_WGT_GRAD == False:
                for idx in range(NUM_FOUND_SOLUTIONS):
                    dw_template(num_cores=1, step='WGT_G')
            # IN_G 1 Core
            if IGNORE_IN_GRAD == False:
                for idx in range(NUM_FOUND_SOLUTIONS):
                    dw_template(num_cores=1, step='IN_G')

            # FW 8 Cores
            if IGNORE_FW == False:
                for idx in range(NUM_FOUND_SOLUTIONS):
                    dw_template(num_cores=8, step='FW')
            # WGT_G 8 Cores
            if IGNORE_WGT_GRAD == False:
                for idx in range(NUM_FOUND_SOLUTIONS):
                    dw_template(num_cores=8, step='WGT_G')
            # IN_G 8 Cores
            if IGNORE_IN_GRAD == False:
                for idx in range(NUM_FOUND_SOLUTIONS):
                    dw_template(num_cores=8, step='IN_G')

        elif layer_type == 'PW':
            # FW 1 Core
            if IGNORE_FW == False:
                for idx in range(NUM_FOUND_SOLUTIONS):
                    pw_template(num_cores=1, step='FW')
            # WGT_G 1 Core
            if IGNORE_WGT_GRAD == False:
                for idx in range(NUM_FOUND_SOLUTIONS):
                    pw_template(num_cores=1, step='WGT_G')
            # IN_G 1 Core
            if IGNORE_IN_GRAD == False:
                for idx in range(NUM_FOUND_SOLUTIONS):
                    pw_template(num_cores=1, step='IN_G')

            # FW 8 Cores
            if IGNORE_FW == False:
                for idx in range(NUM_FOUND_SOLUTIONS):
                    pw_template(num_cores=8, step='FW')
            # WGT_G 8 Cores
            if IGNORE_WGT_GRAD == False:
                for idx in range(NUM_FOUND_SOLUTIONS):
                    pw_template(num_cores=8, step='WGT_G')
            # IN_G 8 Cores
            if IGNORE_IN_GRAD == False:
                for idx in range(NUM_FOUND_SOLUTIONS):
                    pw_template(num_cores=8, step='IN_G')
        yml_f.close()
        py_f.close()
        make_f.close()
        ...
        if IGNORE_FW == False:
            for idx in range(NUM_FOUND_SOLUTIONS):
                lin_template(num_cores=1, step='FW')
        # WGT_G 1 Core
        if IGNORE_WGT_GRAD == False:
            for idx in range(NUM_FOUND_SOLUTIONS):
                lin_template(num_cores=1, step='WGT_G')
        # IN_G 1 Core
        if IGNORE_IN_GRAD == False:
            for idx in range(NUM_FOUND_SOLUTIONS):
                lin_template(num_cores=1, step='IN_G')

        # FW 8 Cores
        if IGNORE_FW == False:
            for idx in range(NUM_FOUND_SOLUTIONS):
                lin_template(num_cores=8, step='FW')
        # WGT_G 8 Cores
        if IGNORE_WGT_GRAD == False:
            for idx in range(NUM_FOUND_SOLUTIONS):
                lin_template(num_cores=8, step='WGT_G')
        # IN_G 8 Cores
        if IGNORE_IN_GRAD == False:
            for idx in range(NUM_FOUND_SOLUTIONS):
                lin_template(num_cores=8, step='IN_G')
        ...
        def conv2d_template(num_cores=1, step='FW'):
            ...
            if step=='FW':
                ...
                step_int = "BACKWARD_ERROR"
            #make_f.write("make profile_all_optim NUM_CORES={} NUM_MATMULS={} IN_CH={} OUT_CH={} STEP='{}' BYPASS=1\n".format(num_cores, NUM_STD_MATMUL, C_in[idx], C_out[idx], step_int))
        ...
        if IGNORE_FW == False:
            for idx in range(NUM_FOUND_SOLUTIONS):
                conv2d_template(num_cores=1, step='FW')
        # WGT_G 1 Core
        if IGNORE_WGT_GRAD == False:
            for idx in range(NUM_FOUND_SOLUTIONS):
                conv2d_template(num_cores=1, step='WGT_G')
        # IN_G 1 Core
        #if IGNORE_IN_GRAD == False:
        #for idx in range(NUM_FOUND_SOLUTIONS):
        #    conv2d_template(num_cores=1, step='IN_G')

        # FW 8 Cores
        if IGNORE_FW == False:
            for idx in range(NUM_FOUND_SOLUTIONS):
                conv2d_template(num_cores=8, step='FW')
        # WGT_G 8 Cores
        if IGNORE_WGT_GRAD == False:
            for idx in range(NUM_FOUND_SOLUTIONS):
                conv2d_template(num_cores=8, step='WGT_G')
        # IN_G 8 Cores
        #if IGNORE_IN_GRAD == False:
        #for idx in range(NUM_FOUND_SOLUTIONS):
        #    conv2d_template(num_cores=8, step='IN_G')
        ...
        if layer_type == 'DW':
            if IGNORE_FW == False:
                ...
                for idx in range(NUM_FOUND_SOLUTIONS):
                    # Results with 1 CORE
                    # FWD
                    os.system("make profile_all_optim NUM_CORES=1 NUM_MATMULS={} IMAGE_H={} IMAGE_W={} DW_KER_H={} DW_KER_W={} DW_IN_CH={} PW_OUT_CH=1 STEP='DW_FORWARD' BYPASS=1".format(NUM_DW_MATMUL, H_in[idx], W_in[idx], KER_H, KER_W, C_in[idx]))
                    write_raw_file(source_file, raw_result_file)
                    tiling_idx, mm, cyc, cores, errors, broken_mm = find_best_perf(source_file, idx, 1)
                    if (errors > 0):
            ...
            if IGNORE_WGT_GRAD == False:
                ...
                for idx in range(NUM_FOUND_SOLUTIONS):
                    # WGT GRAD
                    os.system("make profile_all_optim NUM_CORES=1 NUM_MATMULS={} IMAGE_H={} IMAGE_W={} DW_KER_H={} DW_KER_W={} DW_IN_CH={} PW_OUT_CH=1 STEP='DW_BACKWARD_GRAD' BYPASS=1".format(NUM_DW_MATMUL, H_in[idx], W_in[idx], KER_H, KER_W, C_in[idx]))
                    write_raw_file(source_file, raw_result_file)
                    tiling_idx, mm, cyc, cores, errors, broken_mm = find_best_perf(source_file, idx, 1)
                    if (errors > 0):
            ...
            if IGNORE_IN_GRAD == False:
                ...
                for idx in range(NUM_FOUND_SOLUTIONS):
                    # IN GRAD
                    os.system("make profile_all_optim NUM_CORES=1 NUM_MATMULS={} IMAGE_H={} IMAGE_W={} DW_KER_H={} DW_KER_W={} DW_IN_CH={} PW_OUT_CH=1 STEP='DW_BACKWARD_ERROR' BYPASS=1".format(NUM_DW_MATMUL, H_in[idx], W_in[idx], KER_H, KER_W, C_in[idx]))
                    write_raw_file(source_file, raw_result_file)
                    tiling_idx, mm, cyc, cores, errors, broken_mm = find_best_perf(source_file, idx, 1)
                    if (errors > 0):
            ...
            if IGNORE_FW == False:
                ...
                for idx in range(NUM_FOUND_SOLUTIONS):
                    # Results with 8 CORE
                    # FWD
                    os.system("make profile_all_optim NUM_CORES=8 NUM_MATMULS={} IMAGE_H={} IMAGE_W={} DW_KER_H={} DW_KER_W={} DW_IN_CH={} PW_OUT_CH=1 STEP='DW_FORWARD' BYPASS=1".format(NUM_DW_MATMUL, H_in[idx], W_in[idx], KER_H, KER_W, C_in[idx]))
                    write_raw_file(source_file, raw_result_file)
                    tiling_idx, mm, cyc, cores, errors, broken_mm = find_best_perf(source_file, idx, 8)
                    if (errors > 0):
                        write_error_file(err_log_file, 'FW', tiling_idx, errors, broken_mm)
                    tiling_idx_list.append(tiling_idx); matmul_names_list.append(mm); matmul_cycles_list.append(cyc); num_cores_list.append(cores); passes_list.append('FW')
                sort_results(sim_result_file, tiling_idx_list, matmul_names_list, matmul_cycles_list, num_cores_list, passes_list)
            ...
            if IGNORE_WGT_GRAD == False:
                ...
                for idx in range(NUM_FOUND_SOLUTIONS):
                    # WGT GRAD
                    os.system("make profile_all_optim NUM_CORES=8 NUM_MATMULS={} IMAGE_H={} IMAGE_W={} DW_KER_H={} DW_KER_W={} DW_IN_CH={} PW_OUT_CH=1 STEP='DW_BACKWARD_GRAD' BYPASS=1".format(NUM_DW_MATMUL, H_in[idx], W_in[idx], KER_H, KER_W, C_in[idx]))
                    write_raw_file(source_file, raw_result_file)
                    tiling_idx, mm, cyc, cores, errors, broken_mm = find_best_perf(source_file, idx, 8)
                    if (errors > 0):
                        write_error_file(err_log_file, 'WGT_G', tiling_idx, errors, broken_mm)
            ...
            if IGNORE_IN_GRAD == False:
                ...
                for idx in range(NUM_FOUND_SOLUTIONS):
                    # IN GRAD
                    os.system("make profile_all_optim NUM_CORES=8 NUM_MATMULS={} IMAGE_H={} IMAGE_W={} DW_KER_H={} DW_KER_W={} DW_IN_CH={} PW_OUT_CH=1 STEP='DW_BACKWARD_ERROR' BYPASS=1".format(NUM_DW_MATMUL, H_in[idx], W_in[idx], KER_H, KER_W, C_in[idx]))
                    write_raw_file(source_file, raw_result_file)
                    tiling_idx, mm, cyc, cores, errors, broken_mm = find_best_perf(source_file, idx, 8)
                    if (errors > 0):
                        write_error_file(err_log_file, 'IN_G', tiling_idx, errors, broken_mm)
        ...
        # POINTWISE -------------------------------------------------------------------------------------------------------------------------------------
            ...
            if IGNORE_FW == False:
                ...
                for idx in range(NUM_FOUND_SOLUTIONS):
                    # Results with 1 CORE
                    # FWD
                    os.system("make profile_all_optim NUM_CORES=1 NUM_MATMULS={} IMAGE_H={} IMAGE_W={} DW_KER_H=1 DW_KER_W=1 DW_IN_CH={} PW_OUT_CH={} STEP='PW_FORWARD' BYPASS=1".format(NUM_STD_MATMUL, H_in[idx], W_in[idx], C_in[idx], C_out[idx]))
                    write_raw_file(source_file, raw_result_file)
                    tiling_idx, mm, cyc, cores, errors, broken_mm = find_best_perf(source_file, idx, 1)
                    if (errors > 0):
                        write_error_file(err_log_file, 'FW', tiling_idx, errors, broken_mm)
            ...
            if IGNORE_WGT_GRAD == False:
                ...
                for idx in range(NUM_FOUND_SOLUTIONS):
                    # WGT GRAD
                    os.system("make profile_all_optim NUM_CORES=1 NUM_MATMULS={} IMAGE_H={} IMAGE_W={} DW_KER_H=1 DW_KER_W=1 DW_IN_CH={} PW_OUT_CH={} STEP='PW_BACKWARD_GRAD' BYPASS=1".format(NUM_STD_MATMUL, H_in[idx], W_in[idx], C_in[idx], C_out[idx]))
                    write_raw_file(source_file, raw_result_file)
                    tiling_idx, mm, cyc, cores, errors, broken_mm = find_best_perf(source_file, idx, 1)
                    if (errors > 0):
                        write_error_file(err_log_file, 'WGT_G', tiling_idx, errors, broken_mm)
            ...
            if IGNORE_IN_GRAD == False:
                ...
                for idx in range(NUM_FOUND_SOLUTIONS):
                    # IN GRAD
                    os.system("make profile_all_optim NUM_CORES=1 NUM_MATMULS={} IMAGE_H={} IMAGE_W={} DW_KER_H=1 DW_KER_W=1 DW_IN_CH={} PW_OUT_CH={} STEP='PW_BACKWARD_ERROR' BYPASS=1".format(NUM_STD_MATMUL, H_in[idx], W_in[idx], C_in[idx], C_out[idx]))
                    write_raw_file(source_file, raw_result_file)
                    tiling_idx, mm, cyc, cores, errors, broken_mm = find_best_perf(source_file, idx, 1)
                    if (errors > 0):
                        write_error_file(err_log_file, 'IN_G', tiling_idx, errors, broken_mm)
            ...
            if IGNORE_FW == False:
                ...
                for idx in range(NUM_FOUND_SOLUTIONS):
                    # Results with 8 CORE
                    # FWD
                    os.system("make profile_all_optim NUM_CORES=8 NUM_MATMULS={} IMAGE_H={} IMAGE_W={} DW_KER_H=1 DW_KER_W=1 DW_IN_CH={} PW_OUT_CH={} STEP='PW_FORWARD' BYPASS=1".format(NUM_STD_MATMUL, H_in[idx], W_in[idx], C_in[idx], C_out[idx]))
                    write_raw_file(source_file, raw_result_file)
                    tiling_idx, mm, cyc, cores, errors, broken_mm = find_best_perf(source_file, idx, 8)
                    if (errors > 0):
                        write_error_file(err_log_file, 'FW', tiling_idx, errors, broken_mm)
            ...
            if IGNORE_WGT_GRAD == False:
                ...
                for idx in range(NUM_FOUND_SOLUTIONS):
                    # WGT GRAD
                    os.system("make profile_all_optim NUM_CORES=8 NUM_MATMULS={} IMAGE_H={} IMAGE_W={} DW_KER_H=1 DW_KER_W=1 DW_IN_CH={} PW_OUT_CH={} STEP='PW_BACKWARD_GRAD' BYPASS=1".format(NUM_STD_MATMUL, H_in[idx], W_in[idx], C_in[idx], C_out[idx]))
                    write_raw_file(source_file, raw_result_file)
                    tiling_idx, mm, cyc, cores, errors, broken_mm = find_best_perf(source_file, idx, 8)
                    if (errors > 0):
                        write_error_file(err_log_file, 'WGT_G', tiling_idx, errors, broken_mm)
            ...
            if IGNORE_IN_GRAD == False:
                ...
                for idx in range(NUM_FOUND_SOLUTIONS):
                    # IN GRAD
                    os.system("make profile_all_optim NUM_CORES=8 NUM_MATMULS={} IMAGE_H={} IMAGE_W={} DW_KER_H=1 DW_KER_W=1 DW_IN_CH={} PW_OUT_CH={} STEP='PW_BACKWARD_ERROR' BYPASS=1".format(NUM_STD_MATMUL, H_in[idx], W_in[idx], C_in[idx], C_out[idx]))
                    write_raw_file(source_file, raw_result_file)
                    tiling_idx, mm, cyc, cores, errors, broken_mm = find_best_perf(source_file, idx, 8)
                    if (errors > 0):
                        write_error_file(err_log_file, 'IN_G', tiling_idx, errors, broken_mm)
    ...
    if layer_type == 'LINEAR':
        ...
        if IGNORE_FW == False:
            ...
            for idx in range(NUM_FOUND_SOLUTIONS):
                # Results with 1 CORE
                # FWD
                os.system("make profile_all_optim NUM_CORES=1 NUM_MATMULS={} IN_CH={} OUT_CH={} STEP='FORWARD'".format(NUM_STD_MATMUL, C_in[idx], C_out[idx]))
                write_raw_file(source_file, raw_result_file)
                tiling_idx, mm, cyc, cores, errors, broken_mm = find_best_perf(source_file, idx, 1)
                if (errors > 0):
                    write_error_file(err_log_file, 'FW', tiling_idx, errors, broken_mm)
        ...
        if IGNORE_WGT_GRAD == False:
            ...
            for idx in range(NUM_FOUND_SOLUTIONS):
                # WGT GRAD
                os.system("make profile_all_optim NUM_CORES=1 NUM_MATMULS={} IN_CH={} OUT_CH={} STEP='BACKWARD_GRAD'".format(NUM_STD_MATMUL, C_in[idx], C_out[idx]))
                write_raw_file(source_file, raw_result_file)
                tiling_idx, mm, cyc, cores, errors, broken_mm = find_best_perf(source_file, idx, 1)
                if (errors > 0):
                    write_error_file(err_log_file, 'WGT_G', tiling_idx, errors, broken_mm)
        ...
        if IGNORE_IN_GRAD == False:
            ...
            for idx in range(NUM_FOUND_SOLUTIONS):
                # IN GRAD
                os.system("make profile_all_optim NUM_CORES=1 NUM_MATMULS={} IN_CH={} OUT_CH={} STEP='BACKWARD_ERROR'".format(NUM_STD_MATMUL, C_in[idx], C_out[idx]))
                write_raw_file(source_file, raw_result_file)
                tiling_idx, mm, cyc, cores, errors, broken_mm = find_best_perf(source_file, idx, 1)
                if (errors > 0):
                    write_error_file(err_log_file, 'IN_G', tiling_idx, errors, broken_mm)
        ...
        if IGNORE_FW == False:
            ...
            for idx in range(NUM_FOUND_SOLUTIONS):
                # Results with 8 CORE
                # FWD
                os.system("make profile_all_optim NUM_CORES=8 NUM_MATMULS={} IN_CH={} OUT_CH={} STEP='FORWARD'".format(NUM_STD_MATMUL, C_in[idx], C_out[idx]))
                write_raw_file(source_file, raw_result_file)
                tiling_idx, mm, cyc, cores, errors, broken_mm = find_best_perf(source_file, idx, 8)
                if (errors > 0):
                    write_error_file(err_log_file, 'FW', tiling_idx, errors, broken_mm)
        ...
        if IGNORE_WGT_GRAD == False:
            ...
            for idx in range(NUM_FOUND_SOLUTIONS):
                # WGT GRAD
                os.system("make profile_all_optim NUM_CORES=8 NUM_MATMULS={} IN_CH={} OUT_CH={} STEP='BACKWARD_GRAD'".format(NUM_STD_MATMUL, C_in[idx], C_out[idx]))
                write_raw_file(source_file, raw_result_file)
                tiling_idx, mm, cyc, cores, errors, broken_mm = find_best_perf(source_file, idx, 8)
                if (errors > 0):
                    write_error_file(err_log_file, 'WGT_G', tiling_idx, errors, broken_mm)
        ...
        if IGNORE_IN_GRAD == False:
            ...
            for idx in range(NUM_FOUND_SOLUTIONS):
                # IN GRAD
                os.system("make profile_all_optim NUM_CORES=8 NUM_MATMULS={} IN_CH={} OUT_CH={} STEP='BACKWARD_ERROR'".format(NUM_STD_MATMUL, C_in[idx], C_out[idx]))
                write_raw_file(source_file, raw_result_file)
                tiling_idx, mm, cyc, cores, errors, broken_mm = find_best_perf(source_file, idx, 8)
                if (errors > 0):
                    write_error_file(err_log_file, 'IN_G', tiling_idx, errors, broken_mm)
    ...
    if layer_type == 'CONV2D':
        ...
        if IGNORE_FW == False:
            ...
            for idx in range(NUM_FOUND_SOLUTIONS):
                # Results with 1 CORE
                # FWD
                os.system("make profile_all_optim NUM_CORES=1 NUM_MATMULS={} IN_CH={} OUT_CH={} IMAGE_H={} IMAGE_W={} KER_H={} KER_W={} STEP='FORWARD'".format(NUM_STD_MATMUL, C_in[idx], C_out[idx], H_in[idx], W_in[idx], KER_H, KER_W))
                write_raw_file(source_file, raw_result_file)
                tiling_idx, mm, cyc, cores, errors, broken_mm = find_best_perf(source_file, idx, 1)
                if (errors > 0):
                    write_error_file(err_log_file, 'FW', tiling_idx, errors, broken_mm)
        ...
        if IGNORE_WGT_GRAD == False:
            ...
            for idx in range(NUM_FOUND_SOLUTIONS):
                # WGT GRAD
                os.system("make profile_all_optim NUM_CORES=1 NUM_MATMULS={} IN_CH={} OUT_CH={} IMAGE_H={} IMAGE_W={} KER_H={} KER_W={} STEP='BACKWARD_GRAD'".format(NUM_STD_MATMUL, C_in[idx], C_out[idx], H_in[idx], W_in[idx], KER_H, KER_W))
                write_raw_file(source_file, raw_result_file)
                tiling_idx, mm, cyc, cores, errors, broken_mm = find_best_perf(source_file, idx, 1)
                if (errors > 0):
                    write_error_file(err_log_file, 'WGT_G', tiling_idx, errors, broken_mm)
            ...
            # for idx in range(NUM_FOUND_SOLUTIONS):
            #     # IN GRAD
            #     os.system("make profile_all_optim NUM_CORES=1 NUM_MATMULS={} IN_CH={} OUT_CH={} IMAGE_H={} IMAGE_W={} KER_H={} KER_W={} STEP='BACKWARD_ERROR'".format(NUM_STD_MATMUL, C_in[idx], C_out[idx], H_in[idx], W_in[idx], KER_H, KER_W))
            #     write_raw_file(source_file, raw_result_file)
            #     tiling_idx, mm, cyc, cores, errors, broken_mm = find_best_perf(source_file, idx, 1)
            #     if (errors > 0):
            #         write_error_file(err_log_file, 'IN_G', tiling_idx, errors, broken_mm)
        ...
        if IGNORE_FW == False:
            ...
            for idx in range(NUM_FOUND_SOLUTIONS):
                # Results with 8 CORE
                # FWD
                os.system("make profile_all_optim NUM_CORES=8 NUM_MATMULS={} IN_CH={} OUT_CH={} IMAGE_H={} IMAGE_W={} KER_H={} KER_W={} STEP='FORWARD'".format(NUM_STD_MATMUL, C_in[idx], C_out[idx], H_in[idx], W_in[idx], KER_H, KER_W))
                write_raw_file(source_file, raw_result_file)
                tiling_idx, mm, cyc, cores, errors, broken_mm = find_best_perf(source_file, idx, 8)
                if (errors > 0):
                    write_error_file(err_log_file, 'FW', tiling_idx, errors, broken_mm)
        ...
        if IGNORE_WGT_GRAD == False:
            ...
            for idx in range(NUM_FOUND_SOLUTIONS):
                # WGT GRAD
                os.system("make profile_all_optim NUM_CORES=8 NUM_MATMULS={} IN_CH={} OUT_CH={} IMAGE_H={} IMAGE_W={} KER_H={} KER_W={} STEP='BACKWARD_GRAD'".format(NUM_STD_MATMUL, C_in[idx], C_out[idx], H_in[idx], W_in[idx], KER_H, KER_W))
                write_raw_file(source_file, raw_result_file)
                tiling_idx, mm, cyc, cores, errors, broken_mm = find_best_perf(source_file, idx, 8)
                if (errors > 0):
                    write_error_file(err_log_file, 'WGT_G', tiling_idx, errors, broken_mm)
            ...
            # for idx in range(NUM_FOUND_SOLUTIONS):
            #     # IN GRAD
            #     os.system("make profile_all_optim NUM_CORES=8 NUM_MATMULS={} IN_CH={} OUT_CH={} IMAGE_H={} IMAGE_W={} KER_H={} KER_W={} STEP='BACKWARD_ERROR'".format(NUM_STD_MATMUL, C_in[idx], C_out[idx], H_in[idx], W_in[idx], KER_H, KER_W))
            #     write_raw_file(source_file, raw_result_file)
            #     tiling_idx, mm, cyc, cores, errors, broken_mm = find_best_perf(source_file, idx, 8)
            #     if (errors > 0):
            #         write_error_file(err_log_file, 'IN_G', tiling_idx, errors, broken_mm)
