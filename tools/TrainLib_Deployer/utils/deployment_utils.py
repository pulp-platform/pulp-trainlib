...
def GenerateGM(proj_folder_path, project_name,
    ...
    for layer in range(len(layers_l)):
        ...
        f.write("f.write('#define Tin_C_l"+str(layer)+" "+str(in_ch_l[layer])+"\\n')\n")
        f.write("f.write('#define Tout_C_l"+str(layer)+" "+str(out_ch_l[layer])+"\\n')\n")
        f.write("f.write('#define Tker_H_l"+str(layer)+" "+str(hk_l[layer])+"\\n')\n")
        f.write("f.write('#define Tker_W_l"+str(layer)+" "+str(wk_l[layer])+"\\n')\n")
        f.write("f.write('#define Tin_H_l"+str(layer)+" "+str(hin_l[layer])+"\\n')\n")
        f.write("f.write('#define Tin_W_l"+str(layer)+" "+str(win_l[layer])+"\\n')\n")
        f.write("f.write('#define Tout_H_l"+str(layer)+" "+str(win_l[layer]-hk_l[layer]+1)+"\\n')\n")
        f.write("f.write('#define Tout_W_l"+str(layer)+" "+str(win_l[layer]-wk_l[layer]+1)+"\\n')\n")
        # Padding and stride
        f.write("f.write('#define Tstr_H_l"+str(layer)+" "+str(h_str_l[layer])+"\\n')\n")
        f.write("f.write('#define Tstr_W_l"+str(layer)+" "+str(w_str_l[layer])+"\\n')\n")
        f.write("f.write('#define Tpad_H_l"+str(layer)+" "+str(h_pad_l[layer])+"\\n')\n")
        f.write("f.write('#define Tpad_W_l"+str(layer)+" "+str(w_pad_l[layer])+"\\n')\n")
    f.write("f.close()\n\n")
    ...
    if (layers_l[0] == 'linear'):
        f.write("inp = torch.div(torch.ones("+str(in_ch_l[0])+"), 100000)\n")
    elif (layers_l[0] == 'conv2d' or layers_l[0] == 'DW' or layers_l[0] == 'PW'):
        f.write("inp = torch.torch.div(torch.ones("+str(batch_size)+", "+str(in_ch_l[0])+", "+str(hin_l[0])+", "+str(win_l[0])+"), 1000)\n")
    # Throw error
    else:
    ...
    if (layers_l[-1] == 'linear' or layers_l[-1] == 'ReLU'):
        f.write("label = torch.ones("+str(out_ch_l[-1])+")\n")
    elif (layers_l[0] == 'conv2d' or layers_l[0] == 'DW' or layers_l[0] == 'PW'):
        f.write("label = torch.ones("+str(batch_size)+", "+str(out_ch_l[-1])+", "+str(hin_l[-1]-hk_l[-1]+1)+", "+str(win_l[-1]-wk_l[-1]+1)+")\n")
    # Throw error
    else: 
    ...
    for layer in range(len(layers_l)):
        f.write("f.write('#define WGT_SIZE_L"+str(layer)+" "+str(in_ch_l[layer]*out_ch_l[layer]*hk_l[layer]*wk_l[layer])+"\\n')\n")
        f.write("f.write('PI_L2 float init_WGT_l"+str(layer)+"[WGT_SIZE_L"+str(layer)+"] = {'+dump.tensor_to_string(net.l"+str(layer)+".weight.data)+'};\\n')\n")
    f.write("f.close()\n\n")

    # Define optimizer
    if optimizer == 'SGD':
        f.write("optimizer = optim."+str(optimizer)+"(net.parameters(), lr="+str(learning_rate)+", momentum=0)\n")
    else:
    ...
    out_size = (hin_l[-1]-hk_l[-1]+1) * (win_l[-1]-wk_l[-1]+1) * out_ch_l[-1]
