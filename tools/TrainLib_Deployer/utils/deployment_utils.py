...
def GenerateNet(proj_folder_path, project_name,
    ...
    if data_type == 'FP32':
        ...
        f.write("PI_L1 float BUFF[4*MAX_IN_SIZE + 2*MAX_WGT_SIZE];\n")
        ...
        f.write("PI_L1 fp16 BUFF[4*MAX_IN_SIZE + 2*MAX_WGT_SIZE];\n")
    ...
    f.write("IN_DIFF = BUFF + MAX_IN_SIZE;\n")
    f.write("W_DATA = IN_DIFF + MAX_IN_SIZE;\n")
    f.write("W_DIFF = W_DATA + MAX_WGT_SIZE;\n")
    f.write("OUT_DATA = W_DIFF + MAX_WGT_SIZE;\n")
    f.write("OUT_DIFF = OUT_DATA + MAX_IN_SIZE;\n")
    f.write("update_blob();\n")
    ...
    for layer in range(len(layers_l)):
        ...
        if is_skipderivation:
            f.write(f"\n\tload_output(&layer{lay}_out, 2);\n") # Need to load output blob for diff calculation
        elif lay !=  (len(layers_l) - 1):
            f.write(f"\n\tswap_in_out();\n") # Avoid output load since it's already in L1 memory

        if layers_l[lay] != 'Skipnode':
            f.write(f"\tload_coeff(&layer{lay}_wgt, 1);\n")

        # Load input data
        f.write(f"\tload_input(&layer{lay}_in, 1);\n") 
        ...
        if sumnode_connections[lay] != -1 and layers_l[lay] != 'Sumnode' and layers_l[lay] != 'Skipnode' and skip_in_grad==0:
            f.write("\tswap_in_out();\n")
            f.write(f"\tload_input(&layer{target_layer}_in, 2);\n")
    ...
    if loss_fn == "MSELoss":
        ...
        f.write("  loss_args.target = input_blob.data;\n")
        f.write("  loss_args.wr_loss = &loss;\n")
        f.write(f"  pi_cl_dma_cmd((uint32_t) (LABEL), (uint32_t) (input_blob.data), {float_size}*OUT_SIZE, PI_CL_DMA_DIR_EXT2LOC , cmd_load);\n")
