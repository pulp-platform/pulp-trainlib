...
def linear_template_BW(
    ...
    if SEPARATE_BACKWARD_STEPS == True:
        ...
        if not (FIRST_LAYER == True and UPDATE_LAYER == 0):
            if DATA_TYPE == "FP32":
                template = (
                    "  pulp_linear_fp32_bw_cl(&l" + str(layer_number) + "_args);\n"
                )
            elif DATA_TYPE == "FP16":
                template = (
                    "  pulp_linear_fp16_bw_cl(&l" + str(layer_number) + "_args);\n"
                )
            else:
                print("[net_templates.linear_template_BW]: Invalid data type!")
                exit()
...
def conv2d_template_BW(
    ...
    if SEPARATE_BACKWARD_STEPS == True:
        ...
        if not (FIRST_LAYER == True and UPDATE_LAYER == 0):
            if DATA_TYPE == "FP32":
                template = (
                    "  pulp_conv2d_fp32_bw_cl(&l" + str(layer_number) + "_args);\n"
                )
            elif DATA_TYPE == "FP16":
                template = (
                    "  pulp_conv2d_fp16_bw_cl(&l" + str(layer_number) + "_args);\n"
                )
            else:
                print("[net_templates.conv2d_template_BW]: Invalid data type!")
                exit()
...
def DW_template_BW(
    ...
    if SEPARATE_BACKWARD_STEPS == True:
        ...
        if not (FIRST_LAYER == True and UPDATE_LAYER == 0):
            if DATA_TYPE == "FP32":
                template = (
                    "  pulp_conv_dw_fp32_bw_cl(&l" + str(layer_number) + "_args);\n"
                )
            elif DATA_TYPE == "FP16":
                template = (
                    "  pulp_conv_dw_fp16_bw_cl(&l" + str(layer_number) + "_args);\n"
                )
            else:
                print("[net_templates.DW_template_BW]: Invalid data type!")
                exit()
...
def PW_template_BW(
    ...
    if SEPARATE_BACKWARD_STEPS == True:
        ...
        if not (FIRST_LAYER == True and UPDATE_LAYER == 0):
            if DATA_TYPE == "FP32":
                template = (
                    "  pulp_conv_pw_fp32_bw_cl(&l" + str(layer_number) + "_args);\n"
                )
            elif DATA_TYPE == "FP16":
                template = (
                    "  pulp_conv_pw_fp16_bw_cl(&l" + str(layer_number) + "_args);\n"
                )
            else:
                print("[net_templates.PW_template_BW]: Invalid data type!")
                exit()
...
def LeakyReLU_template_BW(layer_number, DATA_TYPE, FIRST_LAYER):
    ...
    if FIRST_LAYER == False:
        if DATA_TYPE == 'FP32':
            template = "  pulp_leakyrelu_fp32_bw_cl(&l"+str(layer_number)+"_args);\n"
        elif DATA_TYPE == 'FP16':
            template = "  pulp_leakyrelu_fp16_bw_cl(&l"+str(layer_number)+"_args);\n"
        else:
            print("[net_templates.LeakyReLU_template_BW]: Invalid data type!")
            exit()  
    return template


def Sigmoid_template_FW(layer_number, DATA_TYPE):
    if DATA_TYPE == 'FP32':
        template = "  pulp_sigmoid_fp32_fw_cl(&l"+str(layer_number)+"_args);\n"
    elif DATA_TYPE == 'FP16':
        template = "  pulp_sigmoid_fp16_fw_cl(&l"+str(layer_number)+"_args);\n"
    else:
        print("[net_templates.Sigmoid_template_FW]: Invalid data type!")
        exit()  
    return template
...
def Sigmoid_template_BW(layer_number, DATA_TYPE, FIRST_LAYER):
    ...
    if FIRST_LAYER == False:
        if DATA_TYPE == 'FP32':
            template = "  pulp_sigmoid_fp32_bw_cl(&l"+str(layer_number)+"_args);\n"
        elif DATA_TYPE == 'FP16':
            template = "  pulp_sigmoid_fp16_bw_cl(&l"+str(layer_number)+"_args);\n"
        else:
...
def resconn_config_template(layer_number, skip_node, skip_input, layer_type):
    template = (
        "  l" + str(layer_number) + "_args.lout = &layer" + str(layer_number) + "_in;\n"
    )
    template += (
        "  l"
        + str(layer_number)
...
def sum(layer, data_type):
    template = "int dims[] = { layer" + str(layer) + "_in.dim };\n\n"
