...
def GenerateNet(proj_folder_path, project_name,
    ...
    for layer in range(len(layers_l)):
        ...
        if len(layers_l) == 1:
            ...
            if bias_l[layer] == 1:
                ...
                f.write("  layer"+str(layer)+"_wgt.dim = Tout_C_l0;\n")
    ...
    for layer in range(len(layers_l)):
        ...
        if layers_l[lay] == 'linear':
            ...
            f.write(ntemp.conv2d_template_BW(lay, data_type_l[lay], bias_l[lay], SEPARATE_BACKWARD_STEPS, FIRST_LAYER))
    ...
    f.write("\tBIAS_DIFF = BUFF + bias_blob.dim + 2*input_blob.dim;\n")
