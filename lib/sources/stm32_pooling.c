...
void stm32_avgpool_fp32_fw(void * pool_args) {
  ...
  if (Hact!=Ho || Wact!=Wo)   {printf("\n[stm32_avgpool_fp32_fw] Invalid pooling kernel size or output size!\n"); return;}
  int HW = H*W;
  int HWk = Hker*Wker;
  int HWo = Ho*Wo;
...
void stm32_avgpool_fp32_bw(void * pool_args){
  ...
  uint32_t Hact = (H-Hker+Hstr)/Hstr; //H / Hker;
  uint32_t Wact = (W-Wker+Hstr)/Wstr; //W / Wker;
  if (Hact!=Ho || Wact!=Wo)   {printf("\n[stm32_avgpool_fp32_bw] Invalid pooling kernel size or output size!\n"); return;}
  int HW = H*W;
  int HWk = Hker*Wker;
  int HWo = Ho*Wo;
...
void stm32_maxpool_fp32_fw(void * pool_args) {
  ...
  uint32_t Hact = (H-Hker+Hstr)/Hstr; //H / Hker;
  uint32_t Wact = (W-Wker+Hstr)/Wstr; //W / Wker;
  if (Hact!=Ho || Wact!=Wo)   {printf("\n[stm32_maxpool_fp32_fw] Invalid pooling kernel size or output size!\n"); return;}
  int HW = H*W;
  int HWk = Hker*Wker;
  int HWo = Ho*Wo;
...
void stm32_maxpool_fp32_bw(void * pool_args){
  ...
  if (Hact!=Ho || Wact!=Wo)   {printf("\n[stm32_maxpool_fp32_bw] Invalid pooling kernel size or output size!\n"); return;}
  int HW = H*W;
  int HWk = Hker*Wker;
  int HWo = Ho*Wo;
