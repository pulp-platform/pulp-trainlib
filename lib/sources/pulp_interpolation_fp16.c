...
{
    struct interpolation_args_fp16 * args = (struct interpolation_args_fp16 *) interpolation_args_fp16;
    ...
    if (C_in != C_out) {
        printf("[pulp_nearest_neighbour_core_fp16] Input and output channels not matching!!\n");
        return;
    }
...
{
    struct interpolation_args_fp16 * args = (struct interpolation_args_fp16 *) interpolation_args_fp16;
    pi_cl_team_fork(NUM_CORES, pulp_nearest_neighbour_core_fp16, interpolation_args_fp16);
}
        ...
        for (int c = start; c < stop; c++) {
            for (int y = 0; y < H_out; y++) {
                ...
                for (int x = 0; x < W_out; x++) {
                    ...
                    fp16 x_weight = in_x - left_index;

                    if (right_index >= W_in) {
                        right_index = W_in - 1;
                    }

                    fp16 top_interp = inData[c*H_in*W_in + top_index * W_in + left_index] * (1.0f - x_weight)
                                    + inData[c*H_in*W_in + top_index * W_in + right_index] * x_weight;

                    fp16 bottom_interp = inData[c*H_in*W_in + bottom_index * W_in + left_index] * (1.0f - x_weight)
        ...
        for (int c = start; c < stop; c++) {
            for (int y = 0; y < H_out; y++) {
                ...
                for (int x = 0; x < W_out; x++) {
                    ...
                    fp16 top_interp = inDiff[c*H_in*W_in + top_index * W_in + left_index] * (1.0f - x_weight)
                                    + inDiff[c*H_in*W_in + top_index * W_in + right_index] * x_weight;

                    fp16 bottom_interp = inDiff[c*H_in*W_in + bottom_index * W_in + left_index] * (1.0f - x_weight)
...
{
    struct interpolation_args_fp16 * args = (struct interpolation_args_fp16 *) interpolation_args_fp16;
    pi_cl_team_fork(NUM_CORES, pulp_bilinear_core_fp16, interpolation_args_fp16);
}
