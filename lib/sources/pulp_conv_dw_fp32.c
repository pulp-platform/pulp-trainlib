...
void pulp_conv_dw_fp32_fw_cl(struct blob * input, struct blob * coeff, struct blob * output, int pad, float * i2c_buffer, int opt_matmul_type) 
{
  ...
  matMul_args.M = (W_in-pW+1+2*pad)*(H_in-pH+1+2*pad);
...
void pulp_conv_dw_fp32_bw_cl(struct blob * input, struct blob * coeff, struct blob * output, int pad, float * i2c_buffer, int skip_in_grad, int opt_matmul_type_wg, int opt_matmul_type_ig)
{
  pulp_conv_dw_fp32_bw_param_grads_cl(input, coeff, output, pad, i2c_buffer, opt_matmul_type_wg);
  if (skip_in_grad == 0)
  {
    pulp_conv_dw_fp32_bw_input_grads_cl(input, coeff, output, pad, i2c_buffer, opt_matmul_type_ig);
  }
}



void pulp_conv_dw_fp32_bw_param_grads_cl(struct blob * input, struct blob * coeff, struct blob * output, int pad, float * i2c_buffer, int opt_matmul_type)
{
  struct matMul_DW_args matMul_args;
  struct im2col_args im2col_args;
...
void pulp_conv_dw_fp32_bw_input_grads_cl(struct blob * input, struct blob * coeff, struct blob * output, int pad, float * i2c_buffer, int opt_matmul_type)
{
  struct matMul_DW_args matMul_args;
  struct im2col_args im2col_args;
