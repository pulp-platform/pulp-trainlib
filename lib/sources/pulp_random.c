...
float genRandFloat(MTRand* rand) {
  return((float)genRandLong(rand) / (uint32_t)0xffffffff);
}


/**
 * PULP-TrainLib's functions
 */
...
void pulp_random_bernoulli(void * integer_random_args) {
    struct integer_random_args* args = (struct integer_random_args*) integer_random_args;
    int seed = args->seed;
    float prob  = args->probability;
    int* output = args->output;

    // Generate random seed
    float rand = pulp_generate_float_seed(seed);

    // Set output
    if (rand > prob) *output = 0;
    else *output = 1;
