 ...
 * @param void_args 
 */
void stm32_im2col_fp32(void * void_args){
  ...
  if (USE_DMA == 0) {
    ...
    {
      uint32_t Htot, Wtot;
      if ((Hin-Hk+Upad+Dpad+Hstr) % Hstr > 0)     {printf("\n[stm32_im2col_fp32: 243] Invalid H stride (non multiple H sizes): have H_in=%d, H_ker=%d, U_pad=%d, D_pad=%d, H_stride=%d, remainder=%d", Hin, Hk, Upad, Dpad, Hstr, (Hin-Hk+Upad+Dpad+Hstr) % Hstr); return;}
      else                                        Htot = (Hin-Hk+Upad+Dpad+Hstr)/Hstr;
      if ((Win-Wk+Lpad+Rpad+Wstr) % Wstr > 0)     {printf("\n[stm32_im2col_fp32: 243] Invalid W stride (non multiple W sizes): have W_in=%d, W_ker=%d, L_pad=%d, R_pad=%d, W_stride=%d, remainder=%d", Win, Wk, Lpad, Rpad, Wstr, (Win-Wk+Lpad+Rpad+Wstr) % Wstr); return;}
  ...
  // ERROR SIGNAL
    ...
    printf("\n[stm32_im2col_fp32: 414] Invalid USE_DMA parameter (not 0 or 1)\n");
  }
}



void stm32_blocktransp_fp32 (void * void_args)
{
