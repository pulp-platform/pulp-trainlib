  ...
  else 
  {
    ...
    if (transp == 0)
    {
      ...
      for (i=start; i < (stop - 1); i=i+2)
      {
        ...
        if (M & 0x00000003) 
        {
          for(uint32_t l = (M & 0x00000003); l > 0; l = (l - 1))
          {
            for (uint32_t ii=i; ii<i+2; ii++) 
              {
                float left_temp = 0;
                for (uint32_t k=0; k<K; k++)
                {
                  left_temp += A[ii*K+k] * B[k*M+(M-l)];
                }
                C[ii*M+M-l] = left_temp;
              }
          }
        }
      }
