      ...
      for (uint32_t hk=0; hk<pH; hk++) {
        for (uint32_t wk=0; wk<pW; wk++) {
          for (uint32_t ci=0; ci<C_in; ci++) {
            ...
            for (uint32_t ho=0; ho<H_out; ho++) {
              for (uint32_t wo=0; wo<W_out; wo++) {
                  weight_temp += outDiff[wo + ho * W_out + co * H_out * W_out] *
                                 inData[w_str * wo + wk + (h_str * ho + hk) * W_in + ci * H_in * W_in];
                  bias_temp += outDiff[wo + ho * W_out + co * H_out * W_out];
              }
            }
            coeffDiff[wk+hk*pW+ci*pH*pW+co*pH*pW*C_in] = weight_temp;
