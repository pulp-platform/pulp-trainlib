...
void pulp_layer_norm_op_1(void *ln_op1_args) {
    struct layer_norm_op_1 *args = (struct layer_norm_op_1 *) ln_op1_args;

    float *input = args->input;
    float *sum_of_elements = args->sum_of_elements;
    float *sum_of_squared_elements = args->sum_of_squared_elements;
    int size = args->size;
    ...
    for (int i = start; i < stop; i++) {
        sum_of_elements[current_core] += input[i];
        sum_of_squared_elements[current_core] += (input[i] * input[i]);
    }
}


void pulp_layer_norm_op_2(void *ln_op2_args) {
    struct layer_norm_op_2 *args = (struct layer_norm_op_2 *) ln_op2_args;
    ...
    for (int i = start; i < stop; i++) {
        output[i] = ((input[i] - mean[0]) / sqrt_var[0]) * weight[i] + bias[i];
    }
}
