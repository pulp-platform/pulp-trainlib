...
/**
 * @brief Transpose a matrix with specified N, M sizes into another matrix array. 
 * @param void_args (void *) (struct transp_args void_args)
 */
void transpose(void * void_args);

/**
 * @brief Copies an array of size "size" into another destination array. Set up the arguments by using a "struct copy_args" structure. 
 * @param (void * ) (struct copy_args void_args)
 */
void copy (void * void_args);

/**
 * @brief Sets an array of size "size" to a value "value". Set up the arguments by using a "struct set_to_value_args" structure. 
 * @param (void * ) (struct set_to_value_args void_args)
 */
void set_to_value (void * void_args);

/**
 * @brief Selects the matmul to be executed in the selected layer. 
