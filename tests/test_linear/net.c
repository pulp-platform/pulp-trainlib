...
static inline void net_forward(){
  /**  FORWARD FC #1   **/
  #ifdef FORWARD
  pulp_linear_fp32_fw_cl(&layer0_in, &layer0_wgt, &layer0_out);
  #endif
}
...
static inline void train(){
  ...
  #ifdef BACKWARD_ERROR
  pulp_linear_fp32_bw_input_grads_cl(&layer0_in, &layer0_wgt, &layer0_out);
  #endif

  #ifdef BACKWARD_GRAD
  pulp_linear_fp32_bw_param_grads_cl(&layer0_in, &layer0_wgt, &layer0_out);
  #endif
