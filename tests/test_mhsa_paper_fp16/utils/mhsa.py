...
class MultiHeadedSelfAttention(nn.Module):
    ...
    def __init__(self, dim, num_heads, att_dim):
        ...
        self.scaling = (torch.from_numpy(np.asarray((1/math.sqrt(self.head_dim),), dtype=np.float32))).bfloat16()
        self.softmax = nn.Softmax(dim=1)
        self.proj_out = nn.Linear(att_dim, dim, bias=True)
    ...
    def forward(self, x, tgt_len):
        ...
        # OP 3
        scores = torch.bmm(q, k.transpose(1, 2)).bfloat16()

        assert list(scores.size()) == [self.n_heads, tgt_len, tgt_len]

        # OP 4
        scores = scores * (self.scaling).bfloat16()

        # OP 5
        # scores = self.softmax(scores)
        scores = SoftmaxFastExp.apply(scores).bfloat16()

        # OP 6
        scores = torch.bmm(scores, v).bfloat16()
