...
def tensor_to_string(tensor):
    ...
    assert ndim <= 4, "Number of tensor dimensions not supported (shape: {})".format(
        tensor.size()
    )
        ...
        for i in range(sz0):
            for j in range(sz1):
                for k in range(sz2):
                    ...
                    tensor_string += "f, "
                    # if (i*j*k) < (sz0-1)*(sz1-1)*(sz2-1) else 'f'
            ...
            for j in range(sz1):
                for k in range(sz2):
                    for t in range(sz3):
                        ...
                        tensor_string += "f, "
                        # if (i*j*k*t) < (sz0-1)*(sz1-1)*(sz2-1)*(sz3-1) else 'f'
