...
PI_L1 fp16 im2col_buffer_bw[IM2COL_SIZE];
...
static inline void compute_memory_occupation() {
  ...
  // Im2col
  L1_memocc_bytes += Tker_H_l1*Tker_W_l1*Tin_C_l1*(Tout_H_l1)*(Tout_W_l1)*sizeof(fp16);
  // DW Kernel
...
static inline void compute_memory_occupation() {
  ...
  L1_memocc_bytes += Tin_H_l1*Tin_W_l1*Tin_C_l1*sizeof(fp16);
  // Im2col
  L1_memocc_bytes += Tout_H_l1*Tout_W_l1*Tin_C_l1*Tker_H_l1*Tker_W_l1*sizeof(fp16);
  // DW Kernel grad
  L1_memocc_bytes += Tker_H_l1*Tker_W_l1*Tin_C_l1*sizeof(fp16);
  // DW Output grad
  L1_memocc_bytes += Tout_H_l1*Tout_W_l1*Tout_C_l1*sizeof(fp16);
...
static inline void compute_memory_occupation() {
  ...
  L1_memocc_bytes += Tin_H_l1*Tin_W_l1*Tin_C_l1*sizeof(fp16);
  // Im2col
  L1_memocc_bytes += Tker_H_l1*Tker_W_l1*Tout_C_l1*(Tin_H_l1)*(Tin_W_l1)*sizeof(fp16);
  // DW Kernel
  L1_memocc_bytes += Tker_H_l1*Tker_W_l1*Tin_C_l1*sizeof(fp16);
  // DW Output grad
  L1_memocc_bytes += Tout_H_l1*Tout_W_l1*Tout_C_l1*sizeof(fp16);
...
static inline void train(){
  ...
  #ifdef PW_FORWARD
  pulp_conv_pw_fp16_fw_cl(&layer2_in, &layer2_wgt, &layer2_out, Tpad_l2);
  #endif
  ...
  #ifdef PW_BACKWARD_GRAD
  pulp_conv_pw_fp16_bw_param_grads_cl(&layer2_in, &layer2_wgt, &layer2_out, Tpad_l2);
  #endif

  #ifdef PW_BACKWARD_ERROR
  pulp_conv_pw_fp16_bw_input_grads_cl(&layer2_in, &layer2_wgt, &layer2_out, Tpad_l2);
  #endif
