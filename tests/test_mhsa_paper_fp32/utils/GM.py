...
if __name__ == "__main__":
    ...
    net = MyNet(in_h=in_h, in_w=in_w, n_heads=n_heads, att_dim=att_dim).float()
    net.zero_grad()

    grads = net.mhsa.register_full_backward_hook(hook_fn1)

    # ~~~~~~~~~~ MANAGE INPUT ~~~~~~~~~~
    # Generate random input data
    inp = torch.randn(ch_in, in_h, in_w).float()
    ...
    in_wgt_init_tensor_q = torch.randn(att_dim, in_w).float()
    in_wgt_init_tensor_k = torch.randn(att_dim, in_w).float()
    in_wgt_init_tensor_v = torch.randn(att_dim, in_w).float()

    in_bias_init_tensor_q = torch.zeros(att_dim).float()
    in_bias_init_tensor_k = torch.zeros(att_dim).float()
    in_bias_init_tensor_v = torch.zeros(att_dim).float()
    ...
    output_proj_wgt_init_tensor = torch.randn(in_w, att_dim).float()
    output_proj_bias_init_tensor = torch.zeros(in_w).float()
    ...
    f.write(
        "PI_L2 float ATTENTION_OUTPUT_WEIGHTS[OUTPUT_WGT_SIZE] = {"
    ...
    f.write(
        "PI_L2 float OUTPUT[OUTPUT_SIZE] = {" + dump.tensor_to_string(out) + "};\n"
    )
    f.close()
